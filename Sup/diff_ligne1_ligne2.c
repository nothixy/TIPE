...
bool verbose = false;
int cpos[2] = {1, 1};
int passagers_déplacés[2] = {0, 0};
int duree_trajet[2] = {33.75, 75};
...
void* updatetime() {
	while (true) {
		...
		for (int i = 0; i < NB_TRAINS; i++) {
            fprintf(f, "{\"l1\": %d,\"l2\": %d},", passagers_déplacés[0], passagers_déplacés[1]);
			if (trains[i].prochain_depart != 0) {
				trains[i].prochain_depart--;
				if (trains[i].prochain_depart <= ARRET_STATION) {
					trains[i].deplacement = false;
				} else {
					trains[i].deplacement = true;
				}
			} else {
                printf("%d\n", lignes[trains[i].ligne]);
                passagers_déplacés[trains[i].ligne] += 1000 / (lignes[trains[i].ligne].len - 1);
				trains[i].prochain_depart = duree_trajet[trains[i].ligne];
				if (trains[i].backwards) {
					int nxt = indexOf(trains[i].station_actuelle, lignes[trains[i].ligne].stations, trains[i].ligne == 0 ? 3 : 2);
					printf("%d\n", nxt);
					...
					if (trains[i].station_actuelle == trains[i].destination && !trains[i].deplacement) {
						trains[i].destination = lignes[trains[i].ligne].stations[trains[i].ligne == 0 ? 2 : 1 - indexOf(trains[i].destination, lignes[trains[i].ligne].stations, trains[i].ligne == 0 ? 3 : 2)];;
...
int get_train(int x, int y) {
	...
	for (int k=0; k < NB_TRAINS; k++) {
		...
		switch (trains[k].ligne) {
			...
			case 1:
				train_x = 15;
				train_y = indexOf(trains[k].station_actuelle, lignes[trains[k].ligne].stations, 2);
				if (trains[k].backwards) {
					if (x == train_x && y == train_y * 30 - (int) (percentage_done * 30)) {
						train_number = k;
					}
				} else {
					if (x == train_x && y == train_y * 30 + (int) (percentage_done * 30)) {
...
int main(int argc, char* argv[]) {
	...
	for (int i = 0; i < 5; i++) {
		asprintf(&stations.tab[i].nom, "%d", i);
	}
	stations.tab[0].pos[0] = 0;
	stations.tab[0].pos[1] = 0;
	stations.tab[1].pos[0] = 0;
	stations.tab[1].pos[1] = 15;
	stations.tab[2].pos[0] = 0;
	stations.tab[2].pos[1] = 30;
	stations.tab[3].pos[0] = 15;
	stations.tab[3].pos[1] = 0;
	...
	ligne2.len = 2;
	ligne2.nbr = 2;
	ligne2.stations = malloc(2 * sizeof(int));
	ligne2.stations[0] = 2;
	ligne2.stations[2] = 3;
    ...
    for (int i = 0; i < 5; i++) {
        grid[stations.tab[i].pos[0]][stations.tab[i].pos[1]] = i;
    }
	for (int i = 0; i < 1000; i++) {
	...
	for (int i = 0; i < NB_TRAINS; i++) {
        trains[i].ligne = i > 4 ? 0 : 1;
		//trains[i].ligne = ((rand() % 3 + 1 )% 2);
		int st_dest = (int) ((rand() % (trains[i].ligne == 0 ? 2 : 1)) * (trains[i].ligne == 0 ? 2 : 1));
		trains[i].destination = lignes[trains[i].ligne].stations[st_dest];
		trains[i].backwards = (st_dest == 0);
		trains[i].passagers = 0;
		trains[i].station_actuelle = lignes[trains[i].ligne].stations[rand() % (trains[i].ligne == 0 ? 3 : 2)];
		trains[i].deplacement = true;
		trains[i].prochain_depart = rand() % duree_trajet[trains[i].ligne];
	}
	for (int i = 0; i < 5; i++) {
